...
class CDAPack(object):
	...
	def openWindow(self,org):
		filename = self.nb.tab(self.nb.select(), "text")
		dataset = self.file_handler.data[filename]
		lon_var, lat_var = self.file_handler.getLatLon(dataset)
		window = tk.Toplevel(self.root)
		var2 = tk.IntVar(window) # shape file toggle
		var_list = list(self.file_handler.data[filename].data_vars.keys())
		time_range = [str(pd.to_datetime(a).date()) for a in list(self.file_handler.data[filename].variables['time'].values)]
		...
		if (org == 0):
			...
			def callPlotFunction():
				...
				org = varRadio.get()
				if (org == 0):
					xds, geometries = self.file_handler.getShapeData(dataset, var_name, t1, filename, plac_ind)
					plot_maps.plotMapShape(proj_string, xds, lon_var, lat_var, geometries)
				elif (org == 1):
					plot_maps.animation(t2-t1, proj_string,show, save, save_name,lon_var,lat_var,frameRate, self.file_handler.animate_aux(dataset,var_name, t1, filename, plac_ind))
				elif (org == 2):
					xds1, geometries = self.file_handler.getShapeData(dataset, 'u10', t1, filename, plac_ind)
					xds2, geometries = self.file_handler.getShapeData(dataset, 'v10', t1, filename, plac_ind)
					lon_arr = np.sort(((np.array(self.file_handler.data[filename].coords[lon_var]) + 180) % 360) -180)
					lat_arr = np.array(self.file_handler.data[filename].coords[lat_var])
					u_arr = np.array(xds1)
					v_arr = np.array(xds2)
					velocity = np.sqrt(u_arr*u_arr+v_arr*v_arr)
					plot_maps.vectorMap(proj_string, lon_arr, lat_arr, u_arr, v_arr, velocity,varSpin1.get())
				elif (org == 3):
					getU,getV = self.file_handler.animate_aux(dataset,'u10',t1,filename,plac_ind), self.file_handler.animate_aux(dataset,'v10',t1,filename,plac_ind)
					...
					v_arr = np.array(xds2)
					velocity = np.sqrt(u_arr*u_arr+v_arr*v_arr)
					plot_maps.vectorAnim(t2-t1, proj_string,show, save, save_name, getU,getV, frameRate, lon_arr,lat_arr,u_arr, v_arr, velocity, time_range)
			...
			def saveCSV():
				...
				if(varBnd.get() == 0):
					...
					data_arr = self.file_handler.getData(dataset,msgs,outVar,variable = var_name)[1]
					if (filename is not None):
						lat_rn, lon_rn = dataset.coords[lat_var],dataset.coords[lon_var]
						out = self.file_handler.applyShape(data_arr, lat_var, lon_var, filename, plac_ind, lat_r = lat_rn, lon_r = lon_rn)[0]
					else:
						out = data_arr
				elif(varBnd.get() == 1):
					if (filename is not None):
						out = [np.array(self.file_handler.getShapeData(dataset, var_name, time_t[0], filename, plac_ind)[0])]
						for time_i in range(time_t[0], time_t[1]):
							out.append(self.file_handler.getShapeData(dataset,var_name, time_i+1, filename, plac_ind)[0])
			...
			def printShapeData():
				nonlocal window
				masked_data = self.file_handler.getShapeData(dataset,None, None, filename, plac_ind)[0]
				msgs,outVar = self.getIndexes()
				sel_message, output_message = self.file_handler.getData(dataset,msgs,outVar, masked_data = masked_data)
	...
	##########################
		...
		def getStats():
			...
			if(not var_time.get()):
				...
				year_param = None
			if(val_filt.get() == 0):
				outdict = self.file_handler.getStats(dataset, year_param, chk_status, var_arr)
			elif(val_filt.get() == 1):
				...
				lat_r.sort()
				lon_r.sort()
				outdict = self.file_handler.getStats(dataset, year_param, chk_status, var_arr, filt = "bounds", lat_range = slice(lat_r[0], lat_r[1]), lon_range = slice(lon_r[0], lon_r[1]))
			elif(val_filt.get() == 2):
				if(plc_var.get() == "ALL"):
					plac_ind = None
				else:
					plac_ind = places.index(plc_var.get())
				outdict = self.file_handler.getStats(dataset, year_param, chk_status, var_arr, filt = "shapefile", filename = filename, place = plac_ind)
			outField.delete(1.0,tk.END)
			outField.insert(tk.INSERT, self.file_handler.generateMessage(outdict))
	...
	def plotGenerator(self):
		...
		for a in list(self.file_handler.data[filename].dims):
		...
		def dataPlot():
			...
			if (val_filt.get() == 0):
				output_mean, output_std, time_array = self.file_handler.plotData(dataset, start_time_index, end_time_index, time_interval, variables)
			elif(val_filt.get() == 1):
				lat_r = [lat_arr2.index(spn_box1.get()),lat_arr2.index(spn_box2.get())]
				lon_r = [lon_arr2.index(spn_box3.get()),lon_arr2.index(spn_box4.get())]
				lat_r.sort()
				lon_r.sort()
				output_mean, output_std, time_array = self.file_handler.plotData(dataset, start_time_index, end_time_index, time_interval, variables, filt = "bounds", lat_range = slice(lat_r[0], lat_r[1]), lon_range = slice(lon_r[0], lon_r[1]))
			elif(val_filt.get() == 2):
				if(plc_var.get() == "ALL"):
					plac_ind = None
				else:
					plac_ind = places.index(plc_var.get())
				output_mean, output_std, time_array = self.file_handler.plotData(dataset, start_time_index, end_time_index, time_interval, variables, filt = "shapefile", filename = filename, place = plac_ind)
